...
with open(file_to_save, "w") as txt_file:   
     #Print the final vote count to the terminal after opening file.
     election_results = (     
         f"\nElection Results\n"
         f"------------------------\n"
    ...
    # 1. Iterate through the candidate list.
     for candidate_name in candidate_votes:
         # 2. Retrieve vote count and %
         votes = candidate_votes[candidate_name]
        # 3. Calculate % of votes.
         vote_percentage = float(votes) /float(total_votes) * 100
        #4. Print the candidate name and % of votes for each.
        # Print out each candidate's name, vote count and % of # votes to terminal
         candidate_results = (
             f"{candidate_name}: {vote_percentage:.1f}% ({votes:,})\n")
        # Print each candidate's voter count and % to terminal
